...
| Shopify media and fulfillment events: `product_media` + `fulfillment_events` streams | `codex/shopify-media-streams` / `449a400` | `source-shopify:3.5.1-media.1` (`sha256:37c6705e1edcb5407b2e21a689e3a303ba00767910691a6c75a4c8893b98f5c9`) | `bf4d6c1a-6f5a-40fe-8341-02c2fa526b66` | Built and pushed, NOT deployed (source definition unchanged, human-gated) |
...
## Shopify: product media and fulfillment events (3.5.1-media.1)

Problem:

- No Airbyte replacement existed for the Fivetran `media`, `media_image` and `product_media` tables. `product_images` only covers `MediaImage` rows that also have a legacy `Image`, and drops videos, external videos and 3D models.
- No Airbyte replacement existed for the Fivetran `fulfillment_event` table (828,674 rows, 19 columns).

Behavior:

- New `product_media` GraphQL BULK stream over `products.media`, emitting one flat record per media object (`MediaImage`, `Video`, `ExternalVideo`, `Model3d`) with `product_id`, `alt`, `status`, `media_content_type`, flattened `preview_image_{url,width,height}`, and — for `MediaImage` only — `image_{url,width,height}` and `mime_type`. One stream covers all three Fivetran tables. `position` is not exposed by the media connection and is not emitted. `ExternalVideo` does not implement the `File` interface, so `createdAt`/`updatedAt` are selected per concrete type rather than on the interface.
- New `fulfillment_events` REST incremental substream. Shopify exposes fulfillment event history only at `/orders/{order_id}/fulfillments/{fulfillment_id}/events.json`; `Fulfillment.events` is not reachable from a BULK operation. Slices come from the `fulfillments` array already present in the parent `Orders` REST payload, so slice discovery costs no extra API calls; `order_id` is stamped onto each event because the payload carries only `fulfillment_id`.
- Scopes: `product_media` -> `read_products`, `fulfillment_events` -> `read_orders`.
- Cumulative with the payments patch; built from a branch off `codex/shopify-client-credentials`.

Source files: `shopify_graphql/bulk/query.py`, `streams/streams.py`, `source.py`, `scopes.py`, `schemas/{product_media,fulfillment_events}.json`, `unit_tests/{test_source.py,unit_test.py,test_media_and_fulfillment_events.py}`.

Verification: full unit suite 293 passed (the same 2 pre-existing environment-dependent `test_discount_codes_sync` failures); `docker run <image> spec` returns a valid SPEC message; expected stream count 49 -> 51.

Deployment: image pushed only. The Airbyte source definition and connections are deliberately untouched — the version bump is human-gated.
